...
double DescriptionOfCorrect::testRetention(vector<PSMDescription>& testset) {
  ...
  for (size_t ix1 = 0; ix1 < testset.size(); ix1++) {
    double diff = estimateRT(testset[ix1].retentionFeatures)
        - testset[ix1].retentionTime;
    rms += diff * diff;
  }
...
void DescriptionOfCorrect::trainRetention(
                                          vector<PSMDescription>& trainset,
                                          const double C,
                                          const double gamma,
...
void DescriptionOfCorrect::trainCorrect() {
  ...
  if (VERB > 2) cerr << "Description of correct recalibrated, avg pI="
      << avgPI << " avg dM=" << avgDM << endl;
...
void DescriptionOfCorrect::copyModel(svm_model* from) {
  ...
  model->sv_coef = (double **)malloc(sizeof(double *) * (model->nr_class
      - 1));
...
double DescriptionOfCorrect::indexSum(const float* index,
...
double DescriptionOfCorrect::indexAvg(const float* index,
                                      const string& peptide) {
  double sum = 0.0;
  string::const_iterator token = peptide.begin();
  for (; token != peptide.end(); ++token)
...
double DescriptionOfCorrect::indexNearestNeigbourPos(const float* index,
                                                     const string& peptide) {
  double sum = 0.0;
  for (unsigned int ix = 0; ix < peptide.size(); ++ix) {
    if (peptide[ix] == 'R' || peptide[ix] == 'K') {
      if (ix > 0) sum += max(0.0f, index[peptide[ix - 1] - 'A']);
      if (ix < peptide.size() - 1) sum += max(0.0f, index[peptide[ix + 1]
          - 'A']);
    }
  }
  return sum;
...
double DescriptionOfCorrect::indexNearestNeigbourNeg(const float* index,
                                                     const string& peptide) {
  double sum = 0.0;
  for (unsigned int ix = 0; ix < peptide.size(); ++ix) {
...
inline double DescriptionOfCorrect::indexN(const float *index,
                                           const string& peptide) {
  return index[peptide[0] - 'A'];
}

inline double DescriptionOfCorrect::indexC(const float *index,
                                           const string& peptide) {
  return index[peptide[peptide.size() - 1] - 'A'];
}

inline double DescriptionOfCorrect::indexNC(const float *index,
                                            const string& peptide) {
...
double* DescriptionOfCorrect::indexPartialSum(const float* index,
                                              const string& peptide,
                                              const size_t win,
                                              double *features) {
  double sum = 0.0;
...
double* DescriptionOfCorrect::fillFeaturesIndex(const string& peptide,
                                                const float *index,
                                                double *features) {
...
double* DescriptionOfCorrect::fillAAFeatures(const string& pep,
                                             double *feat) {
...
void DescriptionOfCorrect::fillFeaturesAllIndex(const string& pep,
                                                double *features) {
  //  unsigned int ptms=0;
  string peptide = pep;
  ...
  if (!doKlammer) {
    ...
    *(features++) = peptide.size();
    for (vector<unsigned int>::iterator ptm = ptms.begin(); ptm
        != ptms.end(); ++ptm)
      *(features++) = (double)*ptm;
    ...
    features = fillAAFeatures(peptide.substr(peptide.size() - 2, 1),
                              features);
...
float DescriptionOfCorrect::krokhin_index['Z' - 'A' + 1] = { 1.1, 0.0,
                                                             0.45, 0.15,
                                                             0.95, 10.9,
                                                             -0.35, -1.45,
                                                             8.0, 0.0,
                                                             ...
                                                             12.25, 0.0,
                                                             4.85, 0.0 };
float DescriptionOfCorrect::krokhin100_index['Z' - 'A' + 1] = { 1, 0, 0.1,
                                                                0.15, 1,
                                                                11.67,
                                                                -0.35,
                                                                -3.0,
                                                                ...
                                                                13.35, 0,
                                                                5.35, 0 };
float DescriptionOfCorrect::krokhinC2_index['Z' - 'A' + 1] = { 0.5, 0.0,
                                                               0.2, 0.4,
                                                               0, 9.5,
                                                               0.15, -0.2,
                                                               6.6, 0.0,
                                                               ...
                                                               11.8, 0.0,
                                                               4.5, 0.0 };
float DescriptionOfCorrect::krokhinTFA_index['Z' - 'A' + 1] = { 1.11, 0.0,
                                                                0.04,
                                                                -0.22,
                                                                1.08,
                                                                11.34,
...
// negated hessa scale
float DescriptionOfCorrect::hessa_index['Z' - 'A' + 1] = { -0.11, -0.0,
                                                           0.13, -3.49,
                                                           -2.68, 0.32,
                                                           -0.74, -2.06,
                                                           0.60, 0.0,
                                                           ...
                                                           -0.30, 0.0,
                                                           -0.68, 0.0 };
float DescriptionOfCorrect::kytedoolittle_index['Z' - 'A' + 1] = { 1.80,
                                                                   0.0,
                                                                   2.50,
                                                                   -3.50,
                                                                   -3.50,
                                                                   ...
                                                                   -1.30,
                                                                   0.0 };
float DescriptionOfCorrect::aa_weights['Z' - 'A' + 1] = { 71.0788, 0,
                                                          103.1448,
                                                          115.0886,
                                                          129.1155,
                                                          147.1766,
