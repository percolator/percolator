...
void MSReader::writeFile(char* c, MSFileFormat ff, MSObject& m,
  ...
  switch (ff) {
    ...
    case mzXML:
    case mzData:
      cout << "Cannot write mzXML or mzData formats. Nothing written."
          << endl;
      break;
    case bms1:
      setCompression(false);
...
#ifndef _NOSQLITE
void MSReader::writeSqlite(char* c, MSObject& m, char* sha1Report) {
  ...
  char zSql[8192];
  strcpy(zSql,
         "create table msRun(id INTEGER primary key autoincrement not null,"
  ...
  zSql[0] = '\0';
  strcpy(zSql,
         "create table msScan(id INTEGER primary key autoincrement not null,"
  ...
  zSql[0] = '\0';
  strcpy(zSql,
         "create table MS2FileScanCharge(id INTEGER primary key autoincrement not null,"
  ...
  sprintf(zSql,
          "insert into msRun(filename, sha1Sum, creationTime, extractor,extractorVersion, instrumentType) values('%s','%s','%s','%s','%s','%s')",
          c,
          sha1Report,
...
void MSReader::appendFile(char* c, Spectrum& s) {
  ...
  switch (ff) {
    ...
    case psm:
      ...
      break;
    default:
      cout << "Cannot append file: unknown or unsupported file type."
          << endl;
      break;
  }
}
...
void MSReader::appendFile(Spectrum& s) {
  ...
  sprintf(zSql,
          "insert into msScan(runID,startScanNumber,endScanNumber,level,precursorMZ, precursorCharge,retentionTime,fragmentationType,peakCount) "
  ...
  if (s.getMsLevel() > 1) {
    if (s.sizeZ() > 0) {
      for (int i = 0; i < s.sizeZ(); i++) {
        ...
        sprintf(zSql,
                "insert into MS2FileScanCharge(scanID, charge, mass) values(%d, %d, %f)",
                lastScanID,
                chg,
                MH);
        sql_stmt(zSql);
...
#ifndef _NOSQLITE
bool MSReader::readSqlite(char* c, Spectrum& s, int scNum) {
  if (c != NULL) {
    sqlite3_open(c, &db);
    if (db == 0) {
      cout << "Error open database " << c << endl;
...
bool MSReader::executeSqlStmt(Spectrum& s, char* zSql) {
  ...
  if (rc == SQLITE_ROW) {
    ...
    if (find(filter.begin(), filter.end(), msType) != filter.end()) {
      ...
      strcpy(actMethod,
             reinterpret_cast<const char*> (sqlite3_column_text(pStmt, 10)));
      ...
      Byte* comprM = (Byte*)sqlite3_column_blob(pStmt, 14);
      int numBytes2 = sqlite3_column_bytes(pStmt, 15);
      Byte* comprI = (Byte*)sqlite3_column_blob(pStmt, 15);
      getUncompressedPeaks(s,
                           numPeaks,
                           numBytes1,
                           comprM,
                           numBytes2,
...
void MSReader::getUncompressedPeaks(Spectrum& s, int& numPeaks,
...
bool MSReader::readFile(char* c, MSFileFormat f, Spectrum& s, int scNum) {
  ...
  if (scNum != 0) {
    ...
    if (scanHeader.acquisitionNum != scNum) {
      cerr << "ERROR: Found scan number " << scanHeader.acquisitionNum
          << " expecting " << scNum << "." << flush;
      exit(1);
    }
    /*
     switch(filter){
    ...
    if (find(filter.begin(), filter.end(), mslevel) != filter.end()) {
      ...
      if (strlen(scanHeader.activationMethod) > 1) {
        if (strcmp(scanHeader.activationMethod, "CID") == 0) {
          s.setActivationMethod(CID);
        }
        if (strcmp(scanHeader.activationMethod, "ECD") == 0) {
          s.setActivationMethod(ECD);
        }
        if (strcmp(scanHeader.activationMethod, "ETD") == 0) {
          s.setActivationMethod(ETD);
        }
        if (strcmp(scanHeader.activationMethod, "PQD") == 0) {
          s.setActivationMethod(PQD);
        }
        if (strcmp(scanHeader.activationMethod, "HCD") == 0) {
          s.setActivationMethod(HCD);
        } else {
          s.setActivationMethod(na);
        }
      ...
      if (scanHeader.precursorCharge > 0) {
        s.addZState(scanHeader.precursorCharge, scanHeader.precursorMZ
            * scanHeader.precursorCharge
            - (scanHeader.precursorCharge - 1) * 1.00727649);
      }
    ...
    if (scanHeader.precursorCharge > 0) {
      s.addZState(scanHeader.precursorCharge, scanHeader.precursorMZ
          * scanHeader.precursorCharge - (scanHeader.precursorCharge - 1)
          * 1.00727649);
    }
    pPeaks = readPeaks(rampFileIn, pScanIndex[rampIndex]);
    j = 0;
...
void MSReader::setFilter(const MSSpectrumType& m) {
  filter.clear();
  filter.push_back(m);
}
...
void MSReader::writeSpecHeader(FILE* fileOut, bool text, Spectrum& s) {
  ...
  if (text) {
    ...
    if (s.getRTime() > 0) {
      fprintf(fileOut, "I\tRTime\t%.*f\n", 4, s.getRTime());
    }
    if (s.getBPI() > 0) {
      fprintf(fileOut, "I\tBPI\t%.*f\n", 2, s.getBPI());
    }
    if (s.getBPM() > 0) {
      fprintf(fileOut, "I\tBPM\t%.*f\n", 4, s.getBPM());
    }
    if (s.getConversionA() != 0) {
      fprintf(fileOut, "I\tConvA\t%.*f\n", 4, s.getConversionA());
    }
    if (s.getConversionB() != 0) {
      fprintf(fileOut, "I\tConvB\t%.*f\n", 4, s.getConversionB());
    }
    if (s.getTIC() > 0) {
      fprintf(fileOut, "I\tTIC\t%.*f\n", 2, s.getTIC());
    }
    if (s.getIonInjectionTime() > 0) {
      fprintf(fileOut, "I\tIIT\t%.*f\n", 4, s.getIonInjectionTime());
    }
    for (j = 0; j < s.sizeZ(); j++) {
...
MSFileFormat MSReader::checkFileFormat(char* fn) {
  ...
  i = strlen(fn);
  if (strcmp(fn + (i - 4), ".ms1") == 0 || strcmp(fn + (i - 4), ".MS1")
      == 0) {
    return ms1;
  }
  if (strcmp(fn + (i - 4), ".ms2") == 0 || strcmp(fn + (i - 4), ".MS2")
      == 0) {
    return ms2;
  }
  if (strcmp(fn + (i - 3), ".zs") == 0 || strcmp(fn + (i - 3), ".ZS") == 0) {
    return zs;
  }
  if (strcmp(fn + (i - 4), ".uzs") == 0 || strcmp(fn + (i - 4), ".UZS")
      == 0) {
    return uzs;
  }
  if (strcmp(fn + (i - 6), ".msmat") == 0
      || strcmp(fn + (i - 6), ".MSMAT") == 0) {
    return msmat_ff;
  }
  //For now, trust mzXML & mzData also
  if (strcmp(fn + (i - 6), ".mzXML") == 0
      || strcmp(fn + (i - 6), ".mzxml") == 0 || strcmp(fn + (i - 6),
                                                       ".MZXML") == 0) {
    return mzXML;
  }
  if (strcmp(fn + (i - 7), ".mzData") == 0 || strcmp(fn + (i - 7),
                                                     ".mzdata") == 0
      || strcmp(fn + (i - 7), ".MZDATA") == 0) {
    return mzData;
  }
  //add the sqlite3 format
  if (strcmp(fn + (i - 8), ".sqlite3") == 0 || strcmp(fn + (i - 8),
                                                      ".SQlite3") == 0
      || strcmp(fn + (i - 8), ".SQLite3") == 0) {
    return sqlite;
  }
  if (strcmp(fn + (i - 4), ".psm") == 0 || strcmp(fn + (i - 4), ".PSM")
      == 0) {
    return psm;
  }
