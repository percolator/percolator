...
#include "loh/QuickLayerOrderedHeap.hpp"
#include "DataSet.h"
...
void Scores::merge(std::vector<Scores>& sv, double fdr, bool skipNormalizeScores) { //DENNA?
  scores_.clear();
...
void Scores::createXvalSetsBySpectrum(std::vector<Scores>& train, 
  ...
  train.resize(xval_fold, usePi0_);
  test.resize(xval_fold, usePi0_);
...
int Scores::calcScoresSorted(std::vector<double>& w, double fdr, bool skipDecoysPlusOne) {
  ...
  //__gnu_parallel::sort(scores_.begin(), scores_.end(), greater<ScoreHolder> ());
  if (VERB > 3) {
    if (scores_.size() >= 10) {
...
void Scores::generatePositiveTrainingSet(AlgIn& data, const double fdr,
    const double cpos, const bool trainBestPositive) { //EJ SATT
...
void Scores::weedOutRedundant() { //GÖRS EJ?
 ...
 */
void Scores::weedOutRedundantMixMax() { //GÖRS EJ?
...
int Scores::getInitDirection(const double initialSelectionFdr, std::vector<double> &direction)
{
  ...
  {
    for (std::vector<ScoreHolder>::iterator scoreIt = scores_.begin(); scoreIt != scores_.end(); ++scoreIt)
    {
    ...
    positives = calcQ(initialSelectionFdr, skipDecoysPlusOne);
...
unsigned Scores::getQvaluesBelowLevel(double level) {
  ...
  int Scores::calcScoresQuickLOHHelper(const double fdr_threshold, pair<double, bool> *combined_begin, 
  pair<double, bool> *combined_end, int num_tps_at_start_of_layer, int num_fps_at_start_of_layer){
  ...
  int Scores::calcScoresLOHSortSmallLayer(const double fdr_threshold, pair<double, bool> *layer_begin, pair<double, bool> *layer_end, 
  const int num_tps_at_start_of_layer, const int num_fps_at_start_of_layer){
  ...
  int Scores::calcScoresLOH(vector<double>& w, double fdr, bool skipDecoysPlusOne){
