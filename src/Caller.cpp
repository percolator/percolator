...
bool Caller::parseOptions(int argc, char **argv) {
  ...
  intro << "or percolator [options] -P pattern target_and_decoy.sqt"
      << endl;
  intro << "or percolator [options] -g gist.data gist.label" << endl
      << endl;
...
the retention time and difference between observed and calculated mass",
  ...
  if (cmd.optionSet("g")) {
    ...
    if (cmd.arguments.size() != 2) {
      cerr << "Provide exactly two arguments when using gist input"
          << endl;
      exit(-1);
    }
  }
...
void Caller::fillFeatureSets() {
  ...
  printf("dt=%i", DescriptionOfCorrect::totalNumRTFeatures());

  /*
  int aaaaaa =     FeatureNames::getNumFeatures();
  int aaaaaa2;
  if ( docFeatures ) { aaaaaa2 = DescriptionOfCorrect::totalNumRTFeatures(); } else { aaaaaa2 =0;
  };
  pNorm->setSet(all, aaaaaa, aaaaaa2);
  */
...
int Caller::preIterationSetup(vector<vector<double> >& w) {
  ...
  std::cerr << "fullset.size() " << fullset.size() << "FeatureNames::getNumFeatures() + 1" << FeatureNames::getNumFeatures() + 1 << std::endl;
