...
Caller::Caller()
    : pNorm_(NULL),
      ...
      proteinResultFN_(""),
      decoyPsmResultFN_(""),
      decoyPeptideResultFN_(""),
      decoyProteinResultFN_(""),
      analytics_(true),
      useResetAlgorithm_(false),
      useCompositionMatch_(false),
      xmlPrintDecoys_(false),
      xmlPrintExpMass_(true),
      reportUniquePeptides_(true),
      reportPepXML_(false),
      targetDecoyCompetition_(false),
      useMixMax_(false),
      inputSearchType_("auto"),
      selectionFdr_(0.01),
      initialSelectionFdr_(0.01),
      testFdr_(0.01),
      numIterations_(10),
      maxPSMs_(0u),
      nestedXvalBins_(1u),
      selectedCpos_(0.0),
      selectedCneg_(0.0),
      reportEachIteration_(false),
      quickValidation_(false),
...
bool Caller::parseOptions(int argc, char** argv) {
  ...
  intro << "pin.tsv is the tab delimited output file generated by e.g. "
           "sqt2pin;\n";
  intro << "  The tab delimited fields should be:\n";
  intro << "    id <tab> label <tab> scannr <tab> feature1 <tab> ... <tab>\n";
  intro << "    featureN <tab> peptide <tab> proteinId1 <tab> .. <tab> "
           "proteinIdM\n";
  intro << "  Labels are interpreted as 1 -- positive set and test set, -1 -- "
           "negative set.\n";
  intro
  ...
  cmd.defineOption("X", "xmloutput", "Path to xml-output (pout) file.",
                   "filename");
  ...
  cmd.defineOption("Z", "decoy-xml-output",
                   ...
                   "xml-output. Only available if -X is set.",
                   "", TRUE_IF_SET);
  ...
  cmd.defineOption(
      ...
      "vector to evaluate the other PSMs. Recommended when analyzing huge "
      "numbers (>1 million) of PSMs. When set to 0, all PSMs are used for "
      "training as normal. Default = 0.",
      "number");
  cmd.defineOption("x", "quick-validation",
  ...
  cmd.defineOption(
      ...
      "filename");
  cmd.defineOption("k", "xml-in",
                   "Input file given in deprecated pin-xml format generated by "
                   "e.g. sqt2pin with the -k option",
                   "filename");
  ...
  cmd.defineOption(
      ...
      "file. This can be the output of the --weights option from a previous "
      "Percolator analysis. Note that the weights must be in the same order as "
      "the features in the PSM input file(s).",
      "filename");
  cmd.defineOption(Option::NO_SHORT_OPT, "static",
                   "Use the provided initial weights as a static model. If "
                   "used, the --init-weights option must be specified.",
                   "", TRUE_IF_SET);
  cmd.defineOption("V", "default-direction",
                   "Use given feature name as initial search direction, can be "
                   "negated to indicate that a lower value is better.",
                   "[-]?featureName");
  cmd.defineOption(
      "v", "verbose",
  ...
  cmd.defineOption(
      ...
      "option only has an effect if the input PSMs are from separate target "
      "and decoy searches. This is the default setting.",
      "", TRUE_IF_SET);
  cmd.defineOption(
      "Y", "post-processing-tdc",
      "Replace the mix-max method by target-decoy competition for assigning "
      ...
      "option will be turned on automatically, as this is incompatible with "
      "the mix-max method. In case this detection fails, turn this option on "
      "explicitly.",
      "", TRUE_IF_SET);
  cmd.defineOption(
      "I", "search-input",
      "Specify the type of target-decoy search: \"auto\" (Percolator attempts "
      "to detect the search type automatically), \"concatenated\" (single "
      "search on concatenated target-decoy protein db) or \"separate\" (two "
      "searches, one against target and one against decoy protein db). Default "
      "= \"auto\".",
      "value");
  cmd.defineOption("s", "no-schema-validation",
  ...
  cmd.defineOption(
      ...
      "available or protein grouping is not desired, set this flag to \"auto\" "
  ...
  cmd.defineOption(
      ...
      "Define the prefix to identify decoy proteins in the database for the "
      "picked-protein algorithm. This will have no effect on the target/decoy "
      "labels specified in the input file. One of \"auto\" (Percolator guesses "
      "the prefix from the input file) or \"'<DECOY NAME>'\" (search for "
      "decoys using the '<DECOY NAME>' pattern). Default = \"auto\".",
      "value");
  cmd.defineOption(
      "z", "protein-enzyme",
      "Type of enzyme used for in silico protein digestion for picked "
      "protein-level FDR estimation. One of "
      "\"no_enzyme\",\"elastase\",\"pepsin\",\"proteinasek\",\"thermolysin\","
      "\"trypsinp\",\"chymotrypsin\",\"lys-n\",\"lys-c\",\"arg-c\",\"asp-n\","
      "\"glu-c\",\"trypsin\". Default=\"trypsin\".",
      "", "trypsin");
  cmd.defineOption(
      "c", "protein-report-fragments",
      "By default, if the peptides associated with protein A are a proper "
      "subset of the peptides associated with protein B, then protein A is "
      "eliminated and all the peptides are considered as evidence for protein "
      "B. Note that this filtering is done based on the complete set of "
      "peptides in the database, not based on the identified peptides in the "
      "search results. Alternatively, if this option is set and if all of the "
      "identified peptides associated with protein B are also associated with "
      "protein A, then Percolator will report a comma-separated list of "
      "protein IDs, where the full-length protein B is first in the list and "
      "the fragment protein A is listed second. Commas inside protein IDs will "
      "be replaced by semicolons. Not available for Fido.",
      "", TRUE_IF_SET);
  cmd.defineOption(
      ...
      "will be reported as a comma-separated list, instead of the default "
      "behavior of randomly discarding all but one of the proteins. Commas "
   ...
   * change and behavior */
  cmd.defineOption(Option::EXPERIMENTAL_FEATURE, "num-threads",
                   "Number of total parallel threads for SVM training during "
                   "cross validation. Default (one thread per CV fold) = 3.",
                   "value");
  cmd.defineOption(Option::EXPERIMENTAL_FEATURE, "nested-xval-bins",
                   "Number of nested cross validation bins within each cross "
                   "validation bin. This should reduce overfitting of the "
  ...
  cmd.defineOption(
      ...
      "value");
  cmd.defineOption(
      Option::EXPERIMENTAL_FEATURE, "train-best-positive",
      "Enforce that, for each spectrum, at most one PSM is included in the "
      "positive set during each training iteration. If the user only provides "
      "one PSM per spectrum, this filter will have no effect.",
      "", TRUE_IF_SET);
  cmd.defineOption(Option::EXPERIMENTAL_FEATURE, "train-fdr-initial",
                   "Set the FDR threshold for the first iteration. This is "
                   "useful in cases where the original features do not display "
                   "a good separation between targets and decoys. In "
                   "subsequent iterations, the normal --trainFDR will be used.",
                   "value");
  cmd.defineOption(
      Option::EXPERIMENTAL_FEATURE, "parameter-file",
      "Read flags from a parameter file. If flags are specified on the command "
      "line as well, these will override the ones in the parameter file.",
      "filename");
  cmd.defineOption(Option::EXPERIMENTAL_FEATURE, "reset-algorithm",
                   "Run an implementation of the Percolator-RESET Algorithm.",
                   "", TRUE_IF_SET);
  cmd.defineOption(
      Option::EXPERIMENTAL_FEATURE, "composition-match",
      "Run an implementation of the Percolator-RESET psmsAndPeptides with "
  ...
  if (cmd.isOptionSet("only-psms")) {
    ...
    // the different "hacks" below are mainly to keep backwards compatibility
    // with old Mascot versions
    if (cmd.isOptionSet("results-peptides")) {
      if (!cmd.isOptionSet("results-psms")) {
        if (VERB > 0) {
          cerr << "WARNING: The -r/--results-peptides option cannot be used in "
                  "conjunction with -U/--only-psms: no peptide level "
                  "statistics\n"
               << "are calculated, redirecting PSM level statistics to "
        ...
        cerr << "WARNING: The -r/--results-peptides option cannot be used in "
                "conjunction with -U/--only-psms: no peptide level statistics\n"
             << "are calculated, ignoring -r/--results-peptides option."
             << endl;
      }
    }
    if (cmd.isOptionSet("decoy-results-peptides")) {
      if (!cmd.isOptionSet("decoy-results-psms")) {
        if (VERB > 0) {
          cerr << "WARNING: The -B/--decoy-results-peptides option cannot be "
                  "used in conjunction with -U/--only-psms: no peptide level "
                  "statistics\n"
               << "are calculated, redirecting decoy PSM level statistics to "
                  "provided file instead."
               << endl;
        }
        ...
        cerr << "WARNING: The -B/--decoy-results-peptides option cannot be "
                "used in conjunction with -U/--only-psms: no peptide level "
                "statistics\n"
             << "are calculated, ignoring -B/--decoy-results-peptides option."
             << endl;
      }
    }
  } else {
  ...
  if (cmd.isOptionSet("Cneg")) {
    ...
    if (selectedCpos_ == 0) {
      std::cerr
          << "WARNING: the positive penalty(cpos) is 0, therefore both the "
  ...
  if (cmd.isOptionSet("seed")) {
    PseudoRandom::setSeed(
        static_cast<unsigned long int>(cmd.getInt("seed", 1, 20000)));
  }
  if (cmd.isOptionSet("protein-name-separator")) {
    PSMDescription::setProteinNameSeparator(
        cmd.options["protein-name-separator"]);
  }
  ...
  if (cmd.isOptionSet("search-input")) {
    ...
    if (inputSearchType_ == "concatenated") {
      if (useMixMax_) {
        std::cerr
            << "Error: concatenated search specified for -I/--search-input"
  ...
  if (cmd.arguments.size() == 0) {
    if (!cmd.isOptionSet("tab-in") && !cmd.isOptionSet("xml-in") &&
        !cmd.isOptionSet("stdinput-xml") &&
        !cmd.isOptionSet("stdinput-tab")) {  // unless the input comes from -j,
                                             // -k or -e option
  ...
  if (cmd.arguments.size() >= 1) {
    ...
    if (cmd.isOptionSet("xml-in") ||
        cmd.isOptionSet("tab-in")) {  // and if the tab input is also present
      cerr << "Error: use one of either pin-xml or tab-delimited input format.";
      cerr << "\nInvoke with -h option for help.\n";
      return 0;  // ...error
    }
    if (cmd.isOptionSet("stdinput-xml") ||
        cmd.isOptionSet("stdinput-tab")) {  // if stdin pin file is present
      cerr
          << "Error: the pin file has already been given as stdinput argument.";
      cerr << "\nInvoke with -h option for help.\n";
      return 0;  // ...error
    }
  }

  if (cmd.isOptionSet("protein-decoy-pattern"))
    protEstimatorDecoyPrefix_ = cmd.options["protein-decoy-pattern"];
  ...
  if (cmd.isOptionSet("picked-protein")) {
    ...
    if (cmd.isOptionSet("spectral-counting-fdr")) {
      protEstimatorPeptideQvalThreshold =
          cmd.getDouble("spectral-counting-fdr", 0.0, 1.0);
    }
    ...
    if (cmd.isOptionSet("picked-protein")) {
      ...
      if (cmd.isOptionSet("protein-report-fragments"))
        pickedProteinReportFragmentProteins = true;
      if (cmd.isOptionSet("protein-report-duplicates"))
        pickedProteinReportDuplicateProteins = true;

      protEstimator_ = new PickedProteinInterface(
          fastaDatabase, pickedProteinPvalueCutoff,
          pickedProteinReportFragmentProteins,
          pickedProteinReportDuplicateProteins, protEstimatorTrivialGrouping,
          protEstimatorAbsenceRatio, protEstimatorOutputEmpirQVal,
          protEstimatorDecoyPrefix_, protEstimatorPeptideQvalThreshold);
    }
  }
 ...
 * @param isUniquePeptideRun boolean indicating if we want peptide or PSM
 * probabilities
 */
void Caller::calculatePSMProb(Scores& allScores, bool isUniquePeptideRun) {
  ...
  if (isUniquePeptideRun) {
    ...
    if (VERB > 0) {
      std::cerr << "Selected best-scoring PSM per file+scan+expMass"
                << " (target-decoy competition): " << allScores.posSize()
                << " target PSMs and " << allScores.negSize() << " decoy PSMs."
                << std::endl;
    }
  }
  ...
  if (VERB > 0 && writeOutput) {
    ...
    std::cerr << foundPSMs << " target "
              << (reportUniquePeptides_ ? "peptides" : "PSMs") << " with q<"
              << testFdr_ << "." << endl;
    std::cerr << "Calculating posterior error probabilities (PEPs)."
              << std::endl;
  }
  allScores.calcPep(useIrlsPep_, useInterpolatingPep_, usePavaPep_);
  writeResults(allScores, isUniquePeptideRun, writeOutput);
}

void Caller::writeResults(Scores& allScores,
                          bool isUniquePeptideRun,
                          bool writeOutput) {
  if (VERB > 1 && writeOutput) {
    timer.stop();
    std::cerr << "Processing took " << timer.getCPUTimeStr()
              << " cpu seconds or " << timer.getWallTimeStr()
              << " seconds wall clock time." << endl;
  }
...
void Caller::calculateProteinProbabilities(Scores& allScores) {
  ...
  localTimer.stop();
  if (VERB > 1) {
    std::cerr << "Estimating protein probabilities took : "
              << localTimer.getCPUTimeStr() << " cpu seconds or "
              << localTimer.getWallTimeStr() << " seconds wall clock time."
              << endl;
  }
...
void Caller::checkIsWritable(const std::string& filePath) {
  ...
  if (!ofs.is_open()) {
    ...
    temp << "ERROR: Could not open the file " << filePath << " for writing. "
         << "Check if the folder exists and if you have permission to write."
         << std::endl;
...
bool Caller::loadAndNormalizeData(std::istream& dataStream,
  ...
  if (!tabInput_) {
    if (VERB > 1) {
      std::cerr << "Reading pin-xml input from datafile " << inputFN_
                << std::endl;
    }
    success = xmlInterface.readPin(dataStream, inputFN_, setHandler, pCheck_,
                                   protEstimator_, enzyme_);
  } else {
    if (VERB > 1) {
      std::cerr << "Reading tab-delimited input from datafile " << inputFN_
                << std::endl;
    }
  ...
  true search   detected search  mix-max  tdc  flag for mix-max       flag for
  tdc separate      separate         yes      yes  none (but -y allowed)  -Y
  separate      concatenated     yes      yes  -y (force)             -Y
  concatenated  concatenated     no       yes  NA                     none (but
  -Y allowed) concatenated  separate         no       yes  NA -Y (force)
  */
  if (inputSearchType_ == "auto") {
    if (pCheck_->concatenatedSearch()) {
      if (useMixMax_) {
        if (VERB > 0) {
          std::cerr << "Warning: concatenated search input detected, "
                    << "but overridden by -y flag: using mix-max anyway."
                    << std::endl;
        }
      } else {
        if (VERB > 0) {
          if (targetDecoyCompetition_) {
            std::cerr
                ...
                << "competition on Percolator scores." << std::endl;
          } else {
      ...
      if (targetDecoyCompetition_) {  // this also captures the case where input
                                      // was in reality from concatenated search
        if (VERB > 0) {
          std::cerr << "Separate target and decoy search inputs detected, "
                    << "using target-decoy competition on Percolator scores."
                    << std::endl;
        }
      } else {
        useMixMax_ = true;
    ...
    if (VERB > 0) {
      std::cerr << "Warning: concatenated search input detected, but "
                << "overridden by -I flag specifying separate searches."
                << std::endl;
    }
  } else if (!pCheck_->concatenatedSearch() &&
             inputSearchType_ == "concatenated") {
    if (VERB > 0) {
      std::cerr << "Warning: separate searches input detected, but "
                << "overridden by -I flag specifying a concatenated search."
                << std::endl;
    }
  }
  ...
  if (VERB > 0 && useMixMax_ &&
    ...
    std::cerr << "Warning: The mix-max procedure is not well behaved when "
              ...
              << "Consider using target-decoy competition (-Y flag)."
              << std::endl;
  }
  return success;
}
...
//    postToPostHog("percolator_startup");
  ...
  if (!tabFileValidator.validateTabFiles(inputFNs_,
                                         protEstimatorDecoyPrefix_)) {
    return 0;
  }
  ...
  XMLInterface xmlInterface(xmlOutputFN_, pepXMLOutputFN_, xmlSchemaValidation_,
  ...
  if (!loadAndNormalizeData(getDataInStream(fileStream), xmlInterface,
  ...
  timer.stop();
  if (VERB > 1)
    cerr << "Reading in data and feature calculation took "
         << timer.getCPUTimeStr() << " cpu seconds or "
         << timer.getWallTimeStr() << " seconds wall clock time." << endl;
  timer.reset();
  ...
  if (setHandler.getMaxPSMs() > 0u) {
    ...
    if (!tabInput_) {
      success = xmlInterface.readAndScorePin(fileStream, rawWeights, allScores,
                                             inputFN_, setHandler, pCheck_,
                                             protEstimator_, enzyme_);
    } else {
      success = setHandler.readAndScoreTab(fileStream, rawWeights, allScores,
                                           pCheck_);
    }
...
void Caller::calcAndOutputResult(Scores& allScores,
                                 XMLInterface& xmlInterface) {
  ...
  if (reportUniquePeptides_ ||
      ProteinProbEstimator::getCalcProteinLevelProb()) {
