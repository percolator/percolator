...
class Modification {
 public:
  ...
  Modification(const std::string name, double* mass_shift,
               double* neutral_loss, const std::string mod_aas,
               ModificationPosition position, char max_num_per_pep = 3);
 ...
 private:
  void Init(const std::string name, double mass_shift_aver,
