...
bool PSMDescription::isSubPeptide(string& child, string& parent) {
  ...
  if (!(Enzyme::isEnzymatic(parent[0], parent[2])
      && Enzyme::isEnzymatic(parent[len - 3], parent[len - 1]))) return false;
...
void PSMDescription::checkFragmentPeptides(
...
void PSMDescription::setRetentionTime(vector<PSMDescription>& psms, map<
    int, double>& scan2rt) {
  ...
  if (scan2rt.size() == 0) {
    if (psm->retentionTime > 0) {
      ...
      if (VERB > 1) cerr
          << "Approximating retention time with scan number." << endl;
      if (normDiv < 0) {
        double minRT = (double)psm->scan, diffRT = psms.rbegin()->scan
            - psm->scan;
        normDiv = diffRT / 2.;
        normSub = minRT + normDiv;
        if (normDiv == 0.0) normDiv = 1.0;
    ...
    if (normDiv < 0) {
      double minRT = scan2rt.begin()->second, diffRT =
          scan2rt.rbegin()->second - minRT;
      normDiv = diffRT / 2.;
      normSub = minRT + normDiv;
      if (normDiv == 0.0) normDiv = 1.0;
    }
