  ...
  if (param->svm_type == ONE_CLASS || param->svm_type == EPSILON_SVR
    ...
    for (i = 0; i < nr_class; i++)
      for (int j = i + 1; j < nr_class; j++) {
        ...
        if (param->probability) svm_binary_svc_probability(&sub_prob,
                                                           param,
                                                           weighted_C[i],
                                                           weighted_C[j],
                                                           probA[p],
        ...
        f[p] = svm_train_one(&sub_prob,
                             ...
                             weighted_C[j]);
        for (k = 0; k < ci; k++)
          if (!nonzero[si + k] && fabs(f[p].alpha[k]) > 0) nonzero[si + k]
              = true;
        for (k = 0; k < cj; k++)
          if (!nonzero[sj + k] && fabs(f[p].alpha[ci + k]) > 0) nonzero[sj
              + k] = true;
        free(sub_prob.x);
        free(sub_prob.y);
        ++p;
...
void svm_get_labels(const svm_model *model, int* label) {
  if (model->label != NULL) for (int i = 0; i < model->nr_class; i++)
...
double svm_predict_probability(const svm_model *model, const svm_node *x,
  ...
  if ((model->param.svm_type == C_SVC || model->param.svm_type == NU_SVC)
    ...
    int prob_max_idx = 0;
    for (i = 1; i < nr_class; i++)
      if (prob_estimates[i] > prob_estimates[prob_max_idx]) prob_max_idx
          = i;
    for (i = 0; i < nr_class; i++)
...
int svm_save_model(const char *model_file_name, const svm_model *model) {
  ...
  if (param.kernel_type == POLY || param.kernel_type == RBF
      || param.kernel_type == SIGMOID) fprintf(fp,
                                               "gamma %g\n",
                                               param.gamma);

  if (param.kernel_type == POLY || param.kernel_type == SIGMOID) fprintf(fp,
                                                                         "coef0 %g\n",
                                                                         param.coef0);
  ...
  for (int i = 0; i < l; i++) {
    ...
    if (param.kernel_type == PRECOMPUTED)
      ...
      for (int j = 0; j < p->dim; j++)
        if (p->values[j] != 0.0) fprintf(fp, "%d:%.8g ", j, p->values[j]);
#else
    const svm_node *p = SV[i];
...
const char *svm_check_parameter(const svm_problem *prob,
  ...
  if (svm_type != C_SVC && svm_type != NU_SVC && svm_type != ONE_CLASS
      && svm_type != EPSILON_SVR && svm_type != NU_SVR) return "unknown svm type";

  // kernel_type, degree

  int kernel_type = param->kernel_type;
  if (kernel_type != LINEAR && kernel_type != POLY && kernel_type != RBF
      && kernel_type != SIGMOID && kernel_type != PRECOMPUTED) return "unknown kernel type";
  ...
  if (svm_type == NU_SVC || svm_type == ONE_CLASS || svm_type == NU_SVR) if (param->nu
      <= 0 || param->nu > 1) return "nu <= 0 or nu > 1";
