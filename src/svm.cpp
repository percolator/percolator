					...
					const double * sub, const double * div, const size_t *numRetFeatures, const int selected_features, const size_t numRTFeat )
{
	...
	if(fp==NULL)
	{
		printf("Unable to save file %s", model_file_name);
		return -1;
	}
	// write the number of normalized rt features
	fprintf(fp,"numNormalizedFeat %d\n", (*numRetFeatures));
	...
	for(int i = 0; i < (*numRetFeatures); i++)
		fprintf(fp," %lf", sub[i]);
	fprintf(fp, "\n");
	// write the div for the retention time
	fprintf(fp,"div %lf", normDiv);
	// write all the other divs
	for(int i = 0; i < (*numRetFeatures); i++)
		fprintf(fp," %lf", div[i]);
	fprintf(fp, "\n");
					...
					const double * sub, const double * div, const size_t *numRetFeatures)
{
	...
	// write the number of RTFeatures
	fprintf(fp,"numRTFeat %d\n", (*numRetFeatures));
	// write the sub for the retention time
	fprintf(fp,"sub %lf", normSub);
	// write all the other subs
	for(int i = 0; i < (*numRetFeatures); i++)
...
// LOAD A MODEL
// first line: numRTFeat NUMBER
// second line: sub normSub otherSubs
		...
		double * sub, double * div, size_t * numRetFeatures, int * selected_features, size_t * numRTFeat)
{
	...
	// read now the selected features
	fscanf(fp,"%80s",cmd);
	if(strcmp(cmd,"SelectedFeat")==0)
	...
	if(strcmp(cmd,"sub")==0)
	{
		// read the sub for retention time
		fscanf(fp,"%lf", retSub);
	...
	if(strcmp(cmd,"div")==0)
	{
		// read the div for retention time
		fscanf(fp,"%lf", retDiv);
		// read all the other subs
		for(int i = 0; i < (*numRetFeatures); i++)
			 fscanf(fp,"%lf", (div + i));
		...
		double * sub, double * div, size_t * numRetFeatures)
{
	...
	// read first the number of rtFeatures
	char cmd[81];
	fscanf(fp,"%80s",cmd);
	if(strcmp(cmd,"numRTFeat")==0)
		fscanf(fp,"%d",numRetFeatures);
