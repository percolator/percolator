...
/* @(#) $Id: deflate.c,v 1.1 2008/06/14 01:21:44 lukall Exp $ */
...
local int read_bufOF(z_streamp strm, Bytef *buf, unsigned size);
#ifndef FASTEST
#ifdef ASMV
...
#endif
#endif
local uInt longest_match_fastOF(deflate_state *s, IPos cur_match);
...
int ZEXPORT deflateInit2_(strm, level, method, windowBits, memLevel,
  ...
  z_streamp strm;int level;int method;int windowBits;int memLevel;int
  ...
  if (version == Z_NULL || version[0] != my_version[0] || stream_size
      != sizeof(z_stream)) {
...
int ZEXPORT deflateSetDictionary(strm, dictionary, dictLength)
  ...
  if (strm == Z_NULL || strm->state == Z_NULL || dictionary == Z_NULL
      || strm->state->wrap == 2 || (strm->state->wrap == 1
      && strm->state->status != INIT_STATE)) return Z_STREAM_ERROR;

  s = strm->state;
  if (s->wrap) strm->adler = adler32(strm->adler, dictionary, dictLength);
...
int ZEXPORT deflateReset(strm)
  ...
  if (strm == Z_NULL || strm->state == Z_NULL || strm->zalloc
      == (alloc_func)0 || strm->zfree == (free_func)0) {
    return Z_STREAM_ERROR;
  }
...
int ZEXPORT deflateTune(strm, good_length, max_lazy, nice_length,
                        max_chain)
  z_streamp strm;int good_length;int max_lazy;int nice_length;int
      max_chain; {
  deflate_state *s;
...
uLong ZEXPORT deflateBound(strm, sourceLen)
  ...
  destLen = sourceLen + ((sourceLen + 7) >> 3) + ((sourceLen + 63) >> 6)
      + 11;

  /* if can't get parameters, return conservative bound */
  if (strm == Z_NULL || strm->state == Z_NULL) return destLen;

  /* if not default parameters, return conservative bound */
  s = strm->state;
  if (s->w_bits != 15 || s->hash_bits != 8 + 7) return destLen;
...
int ZEXPORT deflate(strm, flush)
  ...
  if (strm == Z_NULL || strm->state == Z_NULL || flush > Z_FINISH || flush
      < 0) {
    return Z_STREAM_ERROR;
  }
  s = strm->state;
  ...
  if (strm->next_out == Z_NULL || (strm->next_in == Z_NULL
      && strm->avail_in != 0) || (s->status == FINISH_STATE && flush
      != Z_FINISH)) {
    ...
    if (s->gzhead->extra != NULL) {
      ...
      while (s->gzindex < (s->gzhead->extra_len & 0xffff)) {
        if (s->pending == s->pending_buf_size) {
          if (s->gzhead->hcrc && s->pending > beg) strm->adler
   ...
   */
  if (strm->avail_in != 0 || s->lookahead != 0 || (flush != Z_NO_FLUSH
      && s->status != FINISH_STATE)) {
    block_state bstate;
...
int ZEXPORT deflateEnd(strm)
  ...
  if (status != INIT_STATE && status != EXTRA_STATE && status
      != NAME_STATE && status != COMMENT_STATE && status != HCRC_STATE
  ...
  deflate_state *s;IPos cur_match; /* current match */
{
  ...
  IPos limit = s->strstart > (IPos)MAX_DIST(s) ? s->strstart
      - (IPos)MAX_DIST(s) : NIL;
  ...
  do {
    ...
    if (match[best_len] != scan_end || match[best_len - 1] != scan_end1
        || *match != *scan || *++match != scan[1]) continue;
     ...
     */
    do {
    } while (*++scan == *++match && *++scan == *++match && *++scan
        == *++match && *++scan == *++match && *++scan == *++match
        && *++scan == *++match && *++scan == *++match && *++scan
        == *++match && scan < strend);
  ...
  deflate_state *s;IPos cur_match; /* current match */
{
  ...
  do {
      ...
      == *++match && *++scan == *++match && *++scan == *++match && *++scan
      == *++match && *++scan == *++match && *++scan == *++match && scan
      < strend);
...
local block_state deflate_fast(s, flush)
  ...
  for (;;) {
    ...
    if (s->match_length >= MIN_MATCH) {
       ...
       */
#ifndef FASTEST
      if (s->match_length <= s->max_insert_length && s->lookahead
          >= MIN_MATCH) {
...
#if TOO_FAR <= 32767
          || (s->match_length == MIN_MATCH && s->strstart - s->match_start
              > TOO_FAR)
#endif
          )) {
