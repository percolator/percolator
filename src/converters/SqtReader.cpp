			...
			std::string psmId,boost::shared_ptr<FragSpectrumScanDatabase> database) 
{
  ...
  while (getline(instr, line)) 
  {
    if (line[0] == 'S') 
    {
      ...
      {
	ostringstream temp;
        temp << "Could not parse the S line: " << line << endl;
	...
	{
	  ostringstream temp;
	  temp << "Could not parse the M line: " << line << endl;
	  throw MyException(temp.str());
        }
  ...
  for(unsigned int ix=0;ix<peptideSequence.size();++ix) 
  {
	...
	additionalAA.find(peptideSequence[ix])==string::npos)
    {
      ...
      {
	ostringstream temp;
	temp << "Peptide sequence " << peptide << " contains modification " 
...
bool SqtReader::checkValidity(const std::string &file)
{
  ...
  {
    ostringstream temp;
    temp << "SQT file not generated by CRUX: " << file << std::endl;
    throw MyException(temp.str());
  }
