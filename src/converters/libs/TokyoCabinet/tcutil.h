...
template<class T>
T _tcmymalloc(size_t size, T dummy)
{
	return (T)MYMALLOC(size)
}

template<class T>
T _tcmycalloc(size_t _NumOfElements, size_t _SizeOfElements, T dummy)
{
	return (T)calloc(_NumOfElements, _SizeOfElements);
}

template<class T>
T _tcmyrealloc(void* _Memory, size_t _NewSize, T dummy)
{
	return (T)realloc(_Memory, _NewSize);
}
#endif
*/
#if defined(__cplusplus)
...
*/#define TCMALLOC(TC_res, TC_size) \
  do { \
    if(!((TC_res) = _tcmymalloc(TC_size, (TC_res)))) tcmyfatal("out of memory"); \
  } while(false)
...
*/#define TCCALLOC(TC_res, TC_nmemb, TC_size) \
  do { \
    if(!((TC_res) = _tcmycalloc((TC_nmemb), (TC_size), (TC_res)))) tcmyfatal("out of memory"); \
  } while(false)
#endif
...
*/#define TCREALLOC(TC_res, TC_ptr, TC_size) \
  do { \
    if(!((TC_res) = _tcmyrealloc((TC_ptr), (TC_size), (TC_res)))) tcmyfatal("out of memory"); \
  } while(false)
#endif
