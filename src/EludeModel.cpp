...
float RTModel::Luna_120_index['Z'-'A'+1] =
          {2.03096, 0.0, -0.69783, 1.36532, 1.85552, 11.48865, -1.13960, -7.04092,
           10.57115, 0.0, -7.19015, 14.97157, 4.76117, -1.37106, 0.0, -0.09888,
           -0.51064, -6.18977, -0.51879, 0.60516, 0.0, 6.16856, 5.27152, 0.0, 2.67505, 0.0};
float RTModel::TFA_index['Z' - 'A' + 1] =
		{5.7081, 0.0, -0.3035, 2.1885, 2.5696, 22.3410, -3.9172, -11.8019, 21.5640, 0.0, -10.9792,
	    29.9424, 7.3929, -2.7548, 0.0, -1.1636, -2.1685, -8.5213, -0.3525, 0.2181, 0.0,
	    15.1851, 11.1008, 0.0, 6.4151, 0.0};
...
float RTModel::cornette_index['Z' - 'A' + 1] =
		{ 0.20, 0.0, 4.10, -3.10, -1.80, 4.40, 0.00, 0.50, 4.80, 0.0, -3.10, 5.70, 4.20, -0.50, 0.0,
		  -2.20, -2.80, 1.40, -0.50, -1.90, 0.0, 4.70, 1.0, 0.0, 3.20, 0.0};
float RTModel::eisenberg_index['Z' - 'A' + 1] =
        { 0.62, 0.0, 0.29, -0.90, -0.74, 1.19, 0.48, -0.40, 1.38, 0.0, -1.50, 1.06, 0.64, -0.78, 0.0,
          0.12, -0.85, -2.53, -0.18, -0.05, 0.0, 1.08, 0.81, 0.0, 0.26, 0.0};
float RTModel::meek_index['Z' - 'A' + 1] =
		{ -0.1, 0.0, -2.2, -2.8, -7.5, 13.9, -0.5, 0.8, 11.8, 0.0, -3.2, 10.0, 7.1, -1.6, 0.0,
		  8.0, -2.5, -4.5, -3.7, 1.5, 0.0, 3.3, 18.1, 0.0, 8.2, 0.0};
...
string RTModel::feature_groups[NO_FEATURE_GROUPS] =
		{ "krokhin_index", "krokhin100_index", "krokhinc2_index", "krokhintfa_index", "doolittle_index",
		  "hessa_index", "peptide_size", "no_ptms", "ptms", "bulkiness", "no_consec_krdenq", "hydrophobic_features", "aa_features", };
// number of features in each group
int RTModel::no_features_per_group[NO_FEATURE_GROUPS] = {17, 17, 17, 17, 17, 17, 1, 1, 3, 1, 1, 2, aaAlphabet.size()};
// groups of features to be used
// krokhin = 1, doolittle = 16, peptide_size = 64, no_ptms = 128, ptms = 256, bulkiness = 512,
// no_consec_krdenq = 1024, hydrophobic features = 2048, aafeatures = 4096
static int DEFAULT_FEATURE_GROUPS = 1 + 16 + 64 + 512 + 1024 + 2048 + 4096;
...
double RTModel::noConsecKRDENQ(const string& peptide)
{
...
double RTModel::noConsecHydrophobic(const string& peptide)
{
	string hydrophobicAA = "AILMFWYVC";
	double noOccurences = 0.0;
	size_t isH1, isH2;
...
double* RTModel::fillHydrophobicFeatures(const string& peptide, double *features)
{
	*(features++) = noHydrophobicAA(peptide);
	*(features++) = noConsecHydrophobic(peptide);
...
double RTModel::indexSumSquaredDiff(const float* index, const string& peptide)
{
...
void RTModel::calcRetentionFeatures(PSMDescription &psm)
{
	...
	if (psm.getRetentionFeatures())
	{
		...
		if (selected_features & 1 << 3)
		{
			...
			//features = fillFeaturesIndex(pep, eisenberg_index, features);
		...
		{
			//features = fillFeaturesIndex(pep, cornette_index, features);
			//features = fillFeaturesIndex(pep, eisenberg_index, features);
...
void RTModel::trainIndexSVRNoCCalibration(vector<PSMDescription> & psms, const double C)
{
	 int noPsms = psms.size();
...
void RTModel::getHydrophobicityIndex(vector<PSMDescription> & psms, const double C)
{
	vector<PSMDescription>::iterator it;
	...
	// train retention
	trainIndexSVRNoCCalibration(psms, C);
...
{
	cerr << "Our hydrophobicity index: " << endl;
	for (int i = 0; i < ('Z' - 'A' + 1); ++i)
		cerr << our_index[i] << "  ";
	cout << endl;
	cout << "-------------------------------" << endl;
