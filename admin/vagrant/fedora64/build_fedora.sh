...
ssh -i insecure_private_key -l root -p 2222 localhost 'bash -s' < rpm_build.sh

#release--------------------------------------------------
cp $tmpdir/build/percolator/per*.rpm $release;
cp $tmpdir/build/converters/per*.rpm $release;
