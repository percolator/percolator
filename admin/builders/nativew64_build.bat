...
set SRC_DIR=%~dp0..\..\..\
set BUILD_DIR=%SRC_DIR%\build-64bit
set RELEASE_DIR=%SRC_DIR%\release-64bit
...
%CMAKE_EXE% -G "Visual Studio %MSVC_VER% Win64" -DXML_SUPPORT=OFF "%SRC_DIR%\percolator"
...
%CMAKE_EXE% -G "Visual Studio %MSVC_VER% Win64" -DCMAKE_PREFIX_PATH="%XERCES_DIR%;%XSD_DIR%" -DXML_SUPPORT=ON "%SRC_DIR%\percolator"
echo build percolator (this will take a few minutes).....
msbuild PACKAGE.vcxproj /p:VCTargetsPath="%VCTARGET%" /p:Configuration=%BUILD_TYPE% /m
