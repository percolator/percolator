...
processFile = os.popen("(" + path + "/percolator -E " + path + 
  "/data/percolator_test/pin.xml 2>&1) > /tmp/percolatorCorrectnessOutput.txt")
exitStatus = processFile.close()
if exitStatus is not None:
...
processFile = os.popen("(" + path + "/percolator -m 2 -E " + path + 
  "/data/percolator_test/pin.xml 2>&1) > /tmp/percolatorCorrectnessOutput.txt")
exitStatus = processFile.close()
if exitStatus is not None:
...
processFile = os.popen("(" + path + "/percolator -D 14 -E " + path + 
  "/data/percolator_test/pin.xml 2>&1) > /tmp/percolatorCorrectnessOutput.txt")
exitStatus = processFile.close()
if exitStatus is not None:
