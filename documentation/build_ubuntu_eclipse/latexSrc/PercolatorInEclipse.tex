...
\begin{itemize}
\item customize the script \texttt{build\_ubuntu\_eclipse\_[XX]bit.sh} by assigning values to the variables under the section "USER MUST SET THESE VARIABLES".
...
\item Install STL support for GDB in order to get human readable prints (pretty-prints) of STL data structures during debugging by running the script \texttt{install\_gdb\_printers.sh} from any location; remember to customize the variables under the section "USER MUST SET THESE VARIABLES". For further details on STL support for GDB, please refer to \url{http://sourceware.org/gdb/wiki/STLSupport}
